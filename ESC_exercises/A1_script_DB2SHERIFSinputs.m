...
extrapolate_slip_rate_option = 1; % 1=tip to 0mm/year, 2 = no zero, 3 = attributes the mean of the mainfault to each section
...
pathout2 = fullfile('A_SHERIFS_CAD','data','CAD_ESC_exercise');
...
Mainfaults = {'CampoFelice';'CerchioPescinaParasano';'FucinoOvindoliPezza';'Magnola';'FucinoMagnola';'MtMarsicano';'MtVelino';'SanSebastiano';'TreMonti';'ValleDelSalto';'Ventrino'}
...
b = resfault_cumsum_length(ind(i+1));
 ...
 elseif extrapolate_slip_rate_option == 3
a=[];b=[]; 
a = resfault_cumsum_length(ind(i));
b = resfault_cumsum_length(ind(i+1));

 meanslipratesectionPreferred(i,1) = AverageSlipRatePreferred;
 meanslipratesectionMin(i,1) = AverageSlipRateMin;
 meanslipratesectionMax(i,1) = AverageSlipRateMax;
