...
% total hazard for a given IML and for a given site.
...
OQ_RUN_ID = '54';% Number of Openquake run ID
...
if(nrlz <100)
HCfaulttab = readtable(fullfile(openquakepath,strcat('hazard_curve-rlz-0',num2str(nrlz),'-PGA_',OQ_RUN_ID,'.csv')),'HeaderLines',1,'HeaderLines',2,'ReadVariableNames',true);
elseif (nrlz <10)
HCfaulttab = readtable(fullfile(openquakepath,strcat('hazard_curve-rlz-00',num2str(nrlz),'-PGA_',OQ_RUN_ID,'.csv')),'HeaderLines',1,'HeaderLines',2,'ReadVariableNames',true);
else
HCfaulttab = readtable(fullfile(openquakepath,strcat('hazard_curve-rlz-',num2str(nrlz),'-PGA_',OQ_RUN_ID,'.csv')),'HeaderLines',1,'HeaderLines',2,'ReadVariableNames',true);
end
...
xlim([0.5 48])
ylim([0 size(PartecipationSections,1)+0.5])
colormap(cool)
cbar = (colorbar)
