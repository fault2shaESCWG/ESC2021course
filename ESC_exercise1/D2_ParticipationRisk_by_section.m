...
if(nrlz <100)
Tot = readtable(fullfile(openquakepath,strcat('damages-rlz-0',num2str(nrlz),'_',OQ_RUN_ID,'.csv')),'HeaderLines',1);
elseif (nrlz <10)
Tot = readtable(fullfile(openquakepath,strcat('damages-rlz-00',num2str(nrlz),'_',OQ_RUN_ID,'.csv')),'HeaderLines',1);
else
Tot = readtable(fullfile(openquakepath,strcat('damages-rlz-',num2str(nrlz),'_',OQ_RUN_ID,'.csv')),'HeaderLines',1,'HeaderLines',2,'ReadVariableNames',true);
end
% inputs
...
%find the row in the hzard curves table corresponding to the site
 dist = sqrt( (Tot.lon-site(1)).^2 + (Tot.lat-site(2)).^2);
...
for j =1:(nrlz-1)
    ...
    T = [];
    if i>=100
        T = readtable(fullfile(openquakepath,strcat('damages-rlz-',num2str(i),'_',OQ_RUN_ID,'.csv ')),'HeaderLines',1,'HeaderLines',2,'ReadVariableNames',true);
    elseif i <100 & i>=10
        T = readtable(fullfile(openquakepath,strcat('damages-rlz-0',num2str(i),'_',OQ_RUN_ID,'.csv ')),'HeaderLines',1,'HeaderLines',2,'ReadVariableNames',true);
        
    elseif i <10
         T = readtable(fullfile(openquakepath,strcat('damages-rlz-00',num2str(i),'_',OQ_RUN_ID,'.csv ')),'HeaderLines',1,'HeaderLines',2,'ReadVariableNames',true);
    end
     
   tabella(j,:) = [i, T.structural_DS5(s(1)),T.structural_DS5(s(2))]; % pre1919 and post2001
    
end    
%
riskofcollapse = Tot.structural_DS5(s(1:2))
...
h = textm(site(:,2),site(:,1)-0.23,sito,'FontSize',6);
